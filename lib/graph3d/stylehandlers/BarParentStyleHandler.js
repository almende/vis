...
support.derive(BarParentStyleHandler, BaseStyleHandler);
 ...
 */
BarParentStyleHandler.prototype.redraw = function(graph3d, ctx, points) {
  ctx.lineJoin = 'round';
  ctx.lineCap  = 'round';
...
BarParentStyleHandler.prototype.adjustForBarWidth =
function(graph3d, xRange, yRange) {
  if (graph3d.defaultXBarWidth !== undefined) {
    graph3d.xBarWidth = graph3d.defaultXBarWidth;
  }
  else {
    var dataX = graph3d.getDistinctValues(data.get(), graph3d.colX);
  ...
  if (graph3d.defaultYBarWidth !== undefined) {
    ...
    var dataY = graph3d.getDistinctValues(data.get(), graph3d.colY);
...
BarParentStyleHandler.prototype.redrawPoint =
...
module.exports = BarParentStyleHandler;
