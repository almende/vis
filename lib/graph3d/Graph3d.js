...
/**
 *
 * Local helper function to tone down the length of the construct:
 *
...
Graph3d.prototype._setBackgroundColor = function(backgroundColor) {
  ...
  if (typeof(backgroundColor) === 'string') {
    ...
    fill        = select(backgroundColor.fill       , fill);
    stroke      = select(backgroundColor.stroke     , stroke);
    strokeWidth = select(backgroundColor.strokeWidth, strokeWidth);
 ...
 * NOTE:
 *
 * * This method was not being called upon starting the style handler
 *   refactoring.
 * * Filled in for completeness.
 *
 * It actually looks superfluous, because:
 *
 * * The point at which is was commented out, is in dataInitialize(). directly
 *   after * the call, the set values are overwritten with other fixed *string*
 *   values.
 *
 * * Not clear what to pass as parameter.
 *
 * TODO: Check if this is necessary; if not remove it and also member var
 *       numberOfColumns in the style handlers.
 *
 * -----------------------------------------------------------------------------
 */
/* DISABLED FOR NOW
...
Graph3d.prototype._dataInitialize = function (rawData, style) {
  ...
  this.xMin = select(this.defaultXMin, xRange.min);
  this.xMax = select(this.defaultXMax, xRange.max);
  if (this.xMax <= this.xMin) this.xMax = this.xMin + 1;
  this.xStep = select(this.defaultXStep, (this.xMax-this.xMin)/5);

  this.yMin = select(this.defaultYMin, yRange.min);
  this.yMax = select(this.defaultYMax, yRange.max);
  if (this.yMax <= this.yMin) this.yMax = this.yMin + 1;
  this.yStep = select(this.defaultYStep, (this.yMax-this.yMin)/5);

  var zRange = this.getColumnRange(data,this.colZ);
  this.zMin = select(this.defaultZMin, zRange.min);
  this.zMax = select(this.defaultZMax, zRange.max);
  if (this.zMax <= this.zMin) this.zMax = this.zMin + 1;
  this.zStep = select(this.defaultZStep, (this.zMax-this.zMin)/5);

  if (this.colValue !== undefined) {
    var valueRange = this.getColumnRange(data,this.colValue);
    this.valueMin = select(this.defaultValueMin, valueRange.min);
    this.valueMax = select(this.defaultValueMax, valueRange.max);
  ...
  this.showLegend = select(this.defaultShowLegend, isLegendGraphStyle);
...
Graph3d.prototype.setOptions = function (options) {
  ...
  if (options !== undefined) {
    ...
    support.safeCopy(options, this, copyFields);
    ...
    if (options.showLegend !== undefined) {
      this.defaultShowLegend = options.showLegend;
    }
    ...
    if (options.cameraPosition !== undefined) {
      var camPos = options.cameraPosition;
      this.camera.setArmRotation(camPos.horizontal, camPos.vertical);
      this.camera.setArmLength(camPos.distance);
    }
...
Graph3d.prototype._redrawFilter = function() {
  ...
  if (this.dataFilter) {
    throw('Call _redrawFilter() only if filter data available.');
  }
...
Graph3d.prototype._redrawInfo = function() {
  ...
  var msg = this.dataFilter.getLabel()
...
Graph3d.prototype._redrawAxis = function() {
  ...
  while (!step.end()) {
    ...
    var msg = '  ' + this.xValueLabel(step.getCurrent()) + '  ';
    ctx.fillText(msg, text.x, text.y);
  ...
  while (!step.end()) {
    ...
    if (this.showGrid) {
      fromPoint = new Point3d(this.xMin, step.getCurrent(), this.zMin);
      toPoint   = new Point3d(this.xMax, step.getCurrent(), this.zMin);

      from = this._convert3Dto2D(fromPoint);
      to   = this._convert3Dto2D(toPoint);
      ...
      fromPoint = new Point3d(this.xMin, step.getCurrent(), this.zMin);
      toPoint   = new Point3d(this.xMin+gridLenY, step.getCurrent(), this.zMin);
      ...
      fromPoint = new Point3d(this.xMax, step.getCurrent(), this.zMin);
      toPoint   = new Point3d(this.xMax-gridLenY, step.getCurrent(), this.zMin);
    ...
    textPoint = new Point3d(xText, step.getCurrent(), this.zMin);
    text      = this._convert3Dto2D(textPoint);
    ...
    ctx.fillStyle = this.axisColor;
    var msg       = '  ' + this.yValueLabel(step.getCurrent()) + '  ';
    ctx.fillText(msg, text.x, text.y);
...
Graph3d.prototype._showTooltip = function (dataPoint) {
  ...
  left = Math.min(maxLeft, this.frame.clientWidth - 10 - contentWidth);
