...
function Graph3d(container, data, options) {
  ...
  support.copy(DEFAULTS, this, OPTIONKEYS);
  ...
  this._setStyle(DEFAULTS.Style, this);
  this._setCameraPosition(DEFAULTS.CameraPosition, this);
  ...
  this._setDataColor(DEFAULTS.DataColor, this);
...
Graph3d.prototype._dataInitialize = function (rawData, style) {
  ...
  this._styleHandler.adjustForBarWidth(this, data, xRange, yRange);

  // calculate minimums and maximums
  this.xMin = select(DEFAULTS.XMin, xRange.min);
  this.xMax = select(DEFAULTS.XMax, xRange.max);
  if (this.xMax <= this.xMin) this.xMax = this.xMin + 1;
  this.xStep = select(DEFAULTS.XStep, (this.xMax-this.xMin)/5);

  this.yMin = select(DEFAULTS.YMin, yRange.min);
  this.yMax = select(DEFAULTS.YMax, yRange.max);
  if (this.yMax <= this.yMin) this.yMax = this.yMin + 1;
  this.yStep = select(DEFAULTS.YStep, (this.yMax-this.yMin)/5);

  var zRange = this.getColumnRange(data,this.colZ);
  this.zMin = select(DEFAULTS.ZMin, zRange.min);
  this.zMax = select(DEFAULTS.ZMax, zRange.max);
  if (this.zMax <= this.zMin) this.zMax = this.zMin + 1;
  this.zStep = select(DEFAULTS.ZStep, (this.zMax-this.zMin)/5);

  if (this.colValue !== undefined) {
    var valueRange = this.getColumnRange(data,this.colValue);
    this.valueMin = select(DEFAULTS.ValueMin, valueRange.min);
    this.valueMax = select(DEFAULTS.ValueMax, valueRange.max);
  ...
  this.showLegend = isLegendGraphStyle || DEFAULTS.ShowLegend;
...
Graph3d.prototype._resizeCenter = function() {
  ...
  this.xcenter = parse(DEFAULTS.XCenter);
  this.ycenter = parse(DEFAULTS.YCenter);
};
 ...
 */
Graph3d.prototype._mergeOptions = function (src, dst) {
  if (src === undefined) {
    return;   // Nothing to do
  }
  ...
  if (src.backgroundColor !== undefined) {
    dst._setBackgroundColor(src.backgroundColor);
  }
...
Graph3d.prototype.setOptions = function (options) {
  this.animationStop();
  this._mergeOptions(options, this);
