...
// TODO
//
// - graph3d/graph3D consitent casing.
//
////////////////////////////////////////////
...
//   Since the whole point of the 3D graphs is to be responsive, this might not be a good idea.
// 
////////////////////////////////////////////

// Start namespace
(function(StyleHandlerstate, $, undefined) {
 ...
 */
function calcTranslations(graph3d, points) {
...
MatrixStyleHandler.prototype.redraw = function(graph3d) {
	var points = graph3d.dataPoints;
  if (points === undefined || points.length <= 0)
    return; // TODO: throw exception?

  var canvas = graph3d.frame.canvas;
...
PointStyleHandler.prototype.getDotColor = function(graph3D, point) {
...
PointStyleHandler.prototype.redrawPoint = function(graph3D, ctx, point, size) {
  ...
  var colors = this.getDotColor(graph3D, point);
...
// Same as for points
BarParentStyleHandler.prototype.getDataPoints = function(graph3d, data) {
	return graph3d._getDataPointsPoints(data);
};
...
BarParentStyleHandler.prototype.redraw = function(graph3d) {
  ...
  for (i = 0; i < points.length; i++) {
    ...
    if (graph3d.style === Graph3d.STYLE.BARCOLOR ) {
      // calculate the color based on the value
      hue = (1 - (point.point.value - graph3d.valueMin) * graph3d.scale.value) * 240;
      color = graph3d._hsv2rgb(hue, 1, 1);
      borderColor = graph3d._hsv2rgb(hue, 1, 0.8);
    }
    else if (graph3d.style === Graph3d.STYLE.BARSIZE) {
      color = graph3d.dataColor.fill;
      borderColor = graph3d.dataColor.stroke;
    }
    else {
      // calculate Hue from the current value. At zMin the hue is 240, at zMax the hue is 0
      hue = (1 - (point.point.z - graph3d.zMin) * graph3d.scale.z  / graph3d.verticalRatio) * 240;
      color = graph3d._hsv2rgb(hue, 1, 1);
      borderColor = graph3d._hsv2rgb(hue, 1, 0.8);
    }

    // calculate size for the bar
    if (graph3d.style === Graph3d.STYLE.BARSIZE) {
      xWidth = (graph3d.xBarWidth / 2) * ((point.point.value - graph3d.valueMin) / (graph3d.valueMax - graph3d.valueMin) * 0.8 + 0.2);
      yWidth = (graph3d.yBarWidth / 2) * ((point.point.value - graph3d.valueMin) / (graph3d.valueMax - graph3d.valueMin) * 0.8 + 0.2);
    }
 ...
 */
DotColorStyleHandler.prototype.getDotColor = function(graph3D, point) {
  // calculate the color based on the value
 ...
 * Override of parent method.
 */
DotSizeStyleHandler.prototype.getDotColor = function(graph3D, point) {
	return {
    color      : graph3D.dataColor.fill,
    borderColor: graph3D.dataColor.stroke
  };
...
// Same as for points
LineStyleHandler.prototype.getDataPoints = function(graph3D, data) {
	return graph3D._getDataPointsPoints(data);
};

LineStyleHandler.prototype.redraw = function(graph3D) {
	return graph3D._redrawDataLine();
};
...
Graph3d.prototype.setOptions = function (options) {
  ...
  if (options !== undefined) {
    ...
    var safe_copy = function(src, dst, fields) {
       for (var i in fields) {
         var field = fields[i];
    ...
    safe_copy(options, this, copy_fields);
...
Graph3d.prototype.redraw = function() {
  ...
  this._redrawAxis();
	...
	this._styleHandler.redraw(this);

  this._redrawInfo();
  this._styleHandler.drawLegend(this);
};
