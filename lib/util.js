 ...
 * If property in b explicitly set to null in b, delete it if `allowDeletion` set.
 * If property not present in b, delete it if `allowDeletion` and `deleteUndefined` set.
 *
 ...
 * @param {boolean} [deleteUndefined=false]  if true, also delete property in a if property is undefined in b 
 * @private
 */
function copyOrDelete(a, b, prop, allowDeletion, deleteUndefined = false) {
  var doDeletion = false;
  if (allowDeletion === true) {
    doDeletion = (b[prop] === undefined && deleteUndefined)
              || (b[prop] === null && a[prop] !== undefined);
  }
...
exports.fillIfDefined = function (a, b, allowDeletion = false) {
  for (var prop in a) {
    // Note that the prototype properties are iterated over as well here.
    if (b[prop] !== undefined) {
      if (b[prop] === null || typeof b[prop] !== 'object') { // Note: typeof null === 'object'
        // NOTE: following copies original logic; however deleteUndefined === true is senseless
        //       due to check on undefined here above
        copyOrDelete(a, b, prop, allowDeletion, true);
      }
      else {
