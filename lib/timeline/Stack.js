 ...
 * @param {{relativeBailingTime: date, bailTimeMs: number, bailFunction: function}} bailOptions
 *            bailing options
 * @param {boolean} group.shouldBailStackItems=false
 *            pointer to the itemset.shouldBailStackItems
 * @return shouldBailStackItems
 */
exports.stack = function(items, margin, force, bailOptions, finishedInitialDraw) {
  if (force) {
    ...
    if (item.stack && item.top === null) {
      ...
      do {
        ...
        for (var j = 0, jj = items.length; j < jj; j++) {
          ...
          if (bailOptions && !finishedInitialDraw) {
