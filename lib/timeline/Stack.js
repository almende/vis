 ...
 * @param {{relativeBailingTime: date, bailTimeMs: number, bailFunction: function, shouldReturn: boolean}} bailOptions
 *            bailing options
 */
exports.stack = function(items, margin, force, bailOptions) {
  if (force) {
    ...
    if (item.stack && item.top === null) {
      ...
      do {
        ...
        for (var j = 0, jj = items.length; j < jj; j++) {
          ...
          if (bailOptions) {
            let { relativeBailingTime, bailTimeMs, bailFunction, shouldReturn } = bailOptions;
            if (Math.abs(new Date() - new Date(relativeBailingTime)) < bailTimeMs ) {
              bailFunction(function(didUserContinue) {
                if (shouldReturn && !didUserContinue) { return; }
              })
            }
          }
