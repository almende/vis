...
exports.stackSubgroups = function(items, margin, subgroups) {
  console.log("stackSubgroups");
  for (var subgroup in subgroups) {
    if (subgroups.hasOwnProperty(subgroup)) {


      subgroups[subgroup].top = margin.axis;
      do {
        // TODO: optimize checking for overlap. when there is a gap without items,
        //       you only need to check for items from the next item on, not from zero
        var collidingItem = null;
        for (var otherSubgroup in subgroups) {
          if (subgroups[otherSubgroup].top !== null && otherSubgroup !== subgroup && exports.collisionByTimes(subgroups[subgroup], subgroups[otherSubgroup])) {
        ...
        if (collidingItem != null) {
          ...
          subgroups[subgroup].top = collidingItem.top + collidingItem.height + margin.item.vertical;
        }
      } while (collidingItem);
    }
  }
  ...
  for (var i = 0; i < items.length; i++) {
    if (items[i].data.subgroup !== undefined) {
      items[i].top = subgroups[items[i].data.subgroup].top + margin.item.vertical;
    }
  }
}
