...
Group.prototype._create = function() {
  ...
  // TODO: revise
  sideLabel.className = 'vis-label';
  this.dom.sideLabel = sideLabel;

  var sideInner = document.createElement('div');
  sideInner.className = 'vis-inner';
  sideLabel.appendChild(sideInner);
  this.dom.sideInner = sideInner;
...
Group.prototype.setData = function(data) {
  ...
  var templateFunction, sideTemplateFunction;
  ...
  if (this.itemSet.options && this.itemSet.options.sideTemplate) {
    sideTemplateFunction = this.itemSet.options.sideTemplate.bind(this);
    sideContent = sideTemplateFunction(data, this.dom.sideInner);
  } else {
    sideContent = data && data.sideContent;
  }

  if (sideContent instanceof Element) {
    this.dom.sideInner.appendChild(sideContent);
    while (this.dom.sideInner.firstChild) {
      this.dom.sideInner.removeChild(this.dom.sideInner.firstChild);
    }
    this.dom.sideInner.appendChild(sideContent);
  } else if (sideContent instanceof Object) {
    sideTemplateFunction(data, this.dom.sideInner);
  } else if (sideContent !== undefined && sideContent !== null) {
    this.dom.sideInner.innerHTML = sideContent;
  } else {
    this.dom.sideInner.innerHTML = this.groupId || ''; // groupId can be null
  }
  ...
  // update side title
  this.dom.sideLabel.title = data && data.sideTitle || '';
  if (!this.dom.sideInner.firstChild) {
    util.addClassName(this.dom.sideInner, 'vis-hidden');
  } else {
    util.removeClassName(this.dom.sideInner, 'vis-hidden');
  }
  ...
  if (className != this.className) {
    if (this.className) {
      ...
      util.removeClassName(this.dom.sideLabel, this.className);
      util.removeClassName(this.dom.foreground, this.className);
      util.removeClassName(this.dom.background, this.className);
      util.removeClassName(this.dom.axis, this.className);
    }
    util.addClassName(this.dom.label, className);
    util.addClassName(this.dom.sideLabel, className);
  ...
  if (this.style) {
    ...
    util.removeCssText(this.dom.sideLabel, this.style);
    this.style = null;
  }
  if (data && data.style) {
    util.addCssText(this.dom.label, data.style);
    util.addCssText(this.dom.sideLabel, data.style);
...
/**
 * Get the width of the side label
 * @return {number} width
 */
Group.prototype.getSideLabelWidth = function() {
  return this.props.sideLabel.width;
};
...
Group.prototype.redraw = function(range, margin, restack) {
  ...
  // recalculate size of sidelabel
  resized = util.updateProperty(this.props.sideLabel, 'width', this.dom.inner.clientWidth) || resized;
  resized = util.updateProperty(this.props.sideLabel, 'height', this.dom.inner.clientHeight) || resized;

  // apply new height
  this.dom.background.style.height  = height + 'px';
  this.dom.foreground.style.height  = height + 'px';
  this.dom.label.style.height = height + 'px';
  this.dom.sideLabel.style.height = height + 'px';
...
Group.prototype._calculateHeight = function (margin) {
  ...
  height = Math.max(height, this.props.label.height, this.props.sideLabel.height);
...
Group.prototype.show = function() {
  ...
  if (!this.dom.sideLabel.parentNode) {
    this.itemSet.dom.sideLabelSet.appendChild(this.dom.sideLabel);
  }
...
Group.prototype.hide = function() {
  ...
  var sideLabel = this.dom.sideLabel;
  if (sideLabel.parentNode) {
    sideLabel.parentNode.removeChild(sideLabel);
  }
