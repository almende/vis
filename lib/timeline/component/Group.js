...
Group.prototype._shouldBailItemsRedraw = function() {
  ...
  var bailOptions = {
    ...
    bailTimeMs: this.itemSet.options.onTimeout && this.itemSet.options.onTimeout.timeoutMs,
    userBailFunction: this.itemSet.options.onTimeout && this.itemSet.options.onTimeout.callback,
    shouldBailStackItems: this.shouldBailStackItems
  };
  var bail = false;
  if (!this.itemSet.initialDrawDone) {
    let { relativeBailingTime, bailTimeMs, userBailFunction, shouldBailStackItems } = bailOptions;
    if (shouldBailStackItems) { return true; }
    if (Math.abs(new Date() - new Date(relativeBailingTime)) > bailTimeMs && !shouldBailStackItems) {
      if (userBailFunction && this.itemSet.userContinueStacking == null) {
        userBailFunction(function(didUserContinue) {
          me.itemSet.userContinueStacking = didUserContinue;
          bail = !didUserContinue;
        })
      } else if (me.itemSet.userContinueStacking == false) {
        bail = true;
      } else {
        bail = false;
      }
  ...
  if (restack) {
    ...
    if (typeof this.itemSet.options.order === 'function') {
      ...
      if (this.doInnerStack && this.itemSet.options.stackSubgroups) {
        ...
        this.shouldBailStackItems = stack.stack(customOrderedItems, margin, true /* restack=true */, this._shouldBailItemsRedraw.bind(this));        
      }
      ...
      if (this.itemSet.options.stack) {
        if (this.doInnerStack && this.itemSet.options.stackSubgroups) {                    
          ...
          this.shouldBailStackItems = stack.stack(this.visibleItems, margin, true /* restack=true */, this._shouldBailItemsRedraw.bind(this));
        }
      } else {
        // no stacking
    ...
    if (this.shouldBailStackItems) {
      this.itemSet.body.emitter.emit('destroy')
    }
