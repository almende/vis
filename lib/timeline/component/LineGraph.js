...
LineGraph.prototype._updateAllGroupData = function (ids, groupIds) {
  if (this.itemsData != null) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      extended.x2 = util.convert(item.x2, 'Date');
...
LineGraph.prototype._convertXcoordinates = function (datapoints) {
  ...
  for (var i = 0; i < datapoints.length; i++) {
    ...
    if (datapoints[i].x2 != undefined) {
      datapoints[i].screen_x2 = toScreen(datapoints[i].x2) + this.props.width;
    }
    else {
      datapoints[i].screen_x2 = undefined;
    }
  }
};
