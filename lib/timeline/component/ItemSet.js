...
ItemSet.prototype._onGroupClick = function (event) {
  ...
  var nestedGroups = groupsData.get(group.nestedGroups).map(function(nestedGroup) {
    ...
    if (group.showNested) {
      ...
      nestedGroup.visible = false;
    }
    return nestedGroup;
  });
  groupsData.update(nestedGroups);
  // this._orderNestedGroups();

  if (!group.showNested) {
    group.dom.collapseGroupBtn.innerHTML = this.options.rtl ? '&#9668' : '&#9658;';
    util.addClassName(group.dom.label, 'collapsed');
  } else {
    group.dom.collapseGroupBtn.innerHTML = '&#9660;';
    util.removeClassName(group.dom.inner, 'collapsed');
  }
}
