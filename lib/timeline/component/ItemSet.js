...
ItemSet.prototype._orderNestedGroups = function(groupIds) {
  ...
  var nestingGroupsIds = this.groupsData.get({
    filter: function(group) {
      return group.nestedGroups
    }
  }).map(function(nestingGroup) { return nestingGroup.id })

  var allNestedGroupsIds = []
  nestingGroupsIds.forEach(function(groupId) { 
    var groupData = me.groupsData.get(groupId); 
    allNestedGroupsIds = allNestedGroupsIds.concat(groupData.nestedGroups)
  })

  var itemsWithoutNestedGroups = groupIds.filter(function(groupId) { return allNestedGroupsIds.indexOf(groupId) == -1 });

  itemsWithoutNestedGroups.forEach(function(groupId){
    newGroupIdsOrder.push(groupId)

    var groupData = me.groupsData.get(groupId);
    if (groupData.nestedGroups) {
      var nestedGroupsIds = me.groupsData.get(groupData.nestedGroups, {
        order: me.options.groupOrder
      }).map(function(group) { return group.id });
      newGroupIdsOrder = newGroupIdsOrder.concat(nestedGroupsIds)
    }
  }, this)
