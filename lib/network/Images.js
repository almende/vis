...
class Images {
    ...
    /**
     * @param {string}      url The key to cache the image against 
     * @param {CachedImage} img The image object
     */    
    _addImageToCache (url, img) {
        var cachedImg = this.images[url];

        if (cachedImg === undefined) {
          cachedImg = img;
        }

        if (cachedImg.initialized()) return; // Skip if already done

        this._fixImageCoordinates(cachedImg.image);
   
        cachedImg.init(); 
        this.images[url] = cachedImg;
    }  
