...
Edge.prototype._label = function (ctx, text, x, y) {
  if (text) {
	...
	var labelAlignment = this.options.labelAlignment;
	var yLine = this.labelDimensions.yLine;
	
	ctx.save();
	
<<<<<<< HEAD
	if(labelAlignment == 'line-center' || labelAlignment == 'line-above'){
=======
	if(labelAlignment == 'line-center' || labelAlignment == 'line-above' || labelAlignment == 'line-below'){
>>>>>>> develop
	...
	this._drawLabelRect(ctx,labelAlignment);
	this._drawLabelText(ctx,x,yLine, lines, lineCount, fontSize, labelAlignment);
...
Edge.prototype._rotateForLabelAlignment = function(ctx) {
	var deltaY = this.from.y - this.to.y;
	var deltaX = this.from.x - this.to.x;
	var angleInDegrees = Math.atan2(deltaY, deltaX);

	// rotate so label it is readable
	if((angleInDegrees < -1 && deltaX < 0) || (angleInDegrees > 0 && deltaX < 0)){
 ...
 * @private
 */
Edge.prototype._drawLabelRect = function(ctx, labelAlignment) {
...
>>>>>>> develop
		if(labelAlignment == 'line-center'){
			ctx.fillRect(-(this.labelDimensions.width/2),
			-(this.labelDimensions.height/2),
			this.labelDimensions.width,
			this.labelDimensions.height);
		}else if(labelAlignment == 'line-above'){
			ctx.fillRect(-(this.labelDimensions.width/2),
<<<<<<< HEAD
			-(this.labelDimensions.height+2),
=======
			-(this.labelDimensions.height+lineMargin),
			this.labelDimensions.width,
			this.labelDimensions.height);
		}else if(labelAlignment == 'line-below'){
			ctx.fillRect(-(this.labelDimensions.width/2),
			lineMargin,
>>>>>>> develop
			this.labelDimensions.width,
 ...
 * @private
 */
Edge.prototype._drawLabelText = function(ctx, x, yLine, lines, lineCount, fontSize, labelAlignment) {

	// draw text
	ctx.fillStyle = this.options.fontColor || "black";
	ctx.textAlign = "center";
	if(labelAlignment == 'line-above'){
