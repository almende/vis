  ...
  */
  ...
  _updateState() {
    ...
    for (nodeId in this.clusteredNodes) {
      let node = this.body.nodes[nodeId];
      if (node === undefined) {
        if (node.isCluster === true) continue;  // regular nodes only
    ...
    for (nodeId in this.body.nodes) {
      ...
      for (n in deletedNodeIds) {
        delete node.containedNodes[deletedNodeIds[n]];
      }
    }
    ...
    // Add the deleted edges to the list
    // This will be both the cluster edges and the clustered edges.
    for (edgeId in this.clusteredEdges) {
      ...
      if (this.body.nodes[edge.fromId] === undefined
       || this.body.nodes[edge.toId]   === undefined) {
        deletedEdgeIds.push(edgeId);
      }
    }

    // Also scan for cluster edges which need to be removed
    ...
    for (nodeId in this.body.nodes) {
      ...
      for (n in deletedEdgeIds) {
        ...
        delete node.containedEdges[deletedEdgeId];

        for (m in node.edges) {
          let edge = node.edges[m];

          if (edge.id === deletedEdgeId) {
            node.edges[m] = null;  // Don't want to directly delete here, because in the loop
            continue;
          }
        ...
        // Clean up the nulls
        node.edges = this._filter(node.edges, function(item) {return item !== null});
      }
    }
    ...
    // TODO: Cluster nodes may now be empty or because of options may not be allowed to contain 1 node
