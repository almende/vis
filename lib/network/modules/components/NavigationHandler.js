...
class NavigationHandler {
  ...
  loadNavigationElements() {
	...
	if (this.freezeAnimationToggle == null){
		this.freezeAnimationToggle = true;
	}
        
    this.navigationDOM = {};
    var navigationDivs = ['up','down','left','right','zoomIn','zoomOut','zoomExtends',this.freezeAnimationToggle? 'freezeAnimation': 'unfreezeAnimation'];
    var navigationDivActions = ['_moveUp','_moveDown','_moveLeft','_moveRight','_zoomIn','_zoomOut','_fit','_freezeAnimation'];
    ...
    for (var i = 0; i < navigationDivs.length; i++) {
	  ...
	  else if (navigationDivActions[i] === '_freezeAnimation'){
		hammerUtil.onTouch(hammer,this._freezeAnimation.bind(this));
	  }
      else {
  ...
  /*
   * this either stops or starts the animation (based on the state of the toggle variable)
   *
   * @private
   */ 
   _freezeAnimation(event) {
	
		if (this.freezeAnimationToggle == undefined){
		   this.freezeAnimationToggle = true;
		}
		// emit either disablePhysics or restorePhysics depending on whether the 
		// button has a Play or Pause picture (controlled by the className below)
		this.body.emitter.emit(this.freezeAnimationToggle?'disablePhysics':'restorePhysics');
		this.body.emitter.emit(this.freezeAnimationToggle?'stopSimulation':'startSimulation');

		event.firstTarget.className = this.freezeAnimationToggle ? "vis-button vis-unfreezeAnimation" : "vis-button vis-freezeAnimation" ;
		this.freezeAnimationToggle = !this.freezeAnimationToggle;
	}
