...
class Node {
   ...
   *
   * This function must also be called also if the global node options have changed and the group options did not.
   *
  ...
  static updateGroupOptions(parentOptions, newOptions, groupList) {
    var group = parentOptions.group;  // newOptions.group;
    ...
    var hasGroup = (typeof group === 'number'
                  || (typeof group === 'string' && group != ''));
    if (!hasGroup) return;

    var groupObj = groupList.get(group);
    util.deepExtend(parentOptions, groupObj);
  ...
  static parseOptions(parentOptions, newOptions, allowDeletion = false, globalOptions = {}, groupList) {
    // Group changes can trigger font changes, so we need to check for this.
    var groupChanged = (newOptions.group !== undefined && parentOptions.group !== newOptions.group);
    ...
    if (groupChanged || (newOptions.font !== undefined && newOptions.font !== null)) {
  ...
  updateLabelModule(options) {
    ...
    this.labelModule.propagateFonts(this.options, options, this.defaultOptions);
