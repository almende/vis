...
class Label {
  ...
  /**
   * Initialize an accumulator object for label processing.
   *
   * This has been moved away from the label processing code for better undestanding upon reading.
   *
   * Note that lines is an Array instance with methods attached.
   *
   * @private
   */
  _createLabelAccumulator() {
    let lines = [];

    lines.add = function(l, text, width, height, values, mod = 'normal') {
      if (this.length == l) {
...
/*
    ...
    lines.addAndAccumulate = function(l, text, width, height, values, mod = "normal") {
      this.add(l, text, width, height, values, mod);
...
*/
  ...
  _processLabelText(ctx, selected, hover, text) {
    ...
    let lines = this._createLabelAccumulator();

    //
    // Formatting values used *only* as input to the accumulator
    // Also, width can be determined directly here
    //
    // Following take formatting values and measure var's out of the loops
    let k = 0;
    let self = this;

    let addText = function(k, text, mod = 'normal') { 
      if (text === undefined) return;

      // TODO: This can be done more efficiently by caching
      let values = self.getFormattingValues(ctx, selected, hover, mod);

      // Note that an empty block will be added!
      let width = 0;
      if (text !== '') {
      ...
      lines.add(k, text, width, values.height, values, mod);
    }


    // Add text in block to current line
    let append = function(text, mod = 'normal') { 
      addText(k, text, mod);
    }

    // Add text in block to current line and start a new line
    let endLine = function(text, mod = 'normal') { 
      addText(k, text, mod);
      k++;
    }
    ...
    let overMaxWidth = function(text) {
      ...
      return (width> self.fontOptions.maxWdt);
    }
    ...
    if (this.elementOptions.font.multi) {
        ...
        if (this.fontOptions.maxWdt > 0) {
          ...
          for (let j = 0; j < blocks.length; j++) {
            ...
            while (w < words.length) {
              ...
              if ((newWidth > this.fontOptions.maxWdt)) {
                endLine(text, mod); 
                text = "";
              } else {
                text = newText;
            ...
            // Add whatever is left after the word loop
            if (text.length > 0) {
              append(text, mod); 
            }
          }
      ...
      if (this.fontOptions.maxWdt > 0) {
        ...
        for (let i = 0; i < lineCount; i++) {
          // Handle words per line

          let words = nlLines[i].split(" ");
          let text = "";

          // Find the largest string smaller than the max width
          let w = 0;
          while (w < words.length) {
            let pre = (text === "") ? "" : " ";
            let newText = text + pre + words[w];
            ...
            let newWidth = ctx.measureText(newText).width;
            if (newWidth> this.fontOptions.maxWdt) {
              // Current string too large; store what we got in a line and
              // continue processing the next words for the new line.
              newLine(text); 
              text = "";
            } else {
              text = newText;
          ...
          // Add whatever is left after the loop to a new line
          if (text.length > 0) {
            newLine(text); 
          }
        }
    ...
    // Determine the sizes of the lines
    ...
    return {width, height, lines: lines};
  }
