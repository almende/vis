...
class Label {
  ...
  _processLabelText(ctx, selected, hover, text) {
    ...
    // Note that lines is an array with methods attached
    let lines = [];

    lines.add = function(l, text, width, height, values, mod) {
      if (this.length == l) {
    ...
    // End def lines as an array with methods attached
    ...
    if (this.elementOptions.font.multi) {
      ...
      for (let i = 0; i < lineCount; i++) {
        ...
        if (blocks.length === 0) {
          ...
          lines.addAndAccumulate(k, "",  0, values.size, values);
        ...
        if (this.fontOptions.maxWdt > 0) {
          ...
          for (let j = 0; j < blocks.length; j++) {
            ...
            let values = this.getFormattingValues(ctx, selected, hover, mod);
            ...
            while (w < words.length) {
              ...
              lastMeasure = measure;
              measure = ctx.measureText(newText);
              ...
              if ((lineWidth + measure.width > this.fontOptions.maxWdt) && (lastMeasure.width != 0)) {
                ...
                lines.add(k, text, lastMeasure.width, values.height, values, mod);
                ...
                if (w === words.length-1) {
                  ...
                  lines.add(k, text, measure.width, values.height, values, mod);
          ...
          for (let j = 0; j < blocks.length; j++) {
            ...
            lines.addAndAccumulate(k, blocks[j].text, measure.width, values.height, values, mod);
            width = lines[k].width > width ? lines[k].width : width;
      ...
      let addText = function(k, text, in_width, values) {
        // Following var's in encompassing function scope.
        // Hence the need to name param 'in_width'.
        lines.addAndAccumulate(k, text, in_width, values.size, values);
        width = lines[k].width > width ? lines[k].width : width;
        height += lines[k].height;
      }
        ...
        for (let i = 0; i < lineCount; i++) {
          ...
          while (w < words.length) {
            ...
            if ((measure.width > this.fontOptions.maxWdt) && (lastMeasure.width != 0)) {
              addText(k, text, lastMeasure.width, values);
              text = "";
              k++;
            } else {
              text = newText;
              if (w === words.length-1) {
                addText(k, text, measure.width, values);
                k++;
              }
              w++;
            }
