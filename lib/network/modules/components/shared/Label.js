  ...
  static parseOptions(parentOptions, newOptions, allowDeletion = false) {
    ...
    if (Label.parseFontString(parentOptions, newFont)) {
      ...
      for (var n in parentOptions) {
        ...
        if (typeof parentMultiFont !== 'object' || parentMultiFont.mod === undefined) continue;

        var newMultiFont = newFont[n];
        if (newMultiFont === undefined) continue;  // Doesn't need to be present

        // multi-fonts can be in string form as well, this should have been copied OK
...
/*
        if (Label.parseFontString(parentMultiFont, newMultiFont)) {
          continue;  // for use as breakpoint trap
        }
*/
   ...
   *
   * @param {Object} nodeOptions     the global node options
   * @param {Object} newOptions  the new options to set
   * @param {Object} defaultOptions
   */
  propagateFonts(nodeOptions, newOptions, defaultOptions) {
    // NOTE: 'this.fontOptions' is the instance variable containing the *instance-local* font options
    if (!this.fontOptions.multi) return;

    // The order of inheritance for multi-font properties should be (bold taken as example):
    //
    ...
    //
    // Notes:
    // - Absent here are the group-options!
    //   TODO: examine where they come in
    // - prototype parents over first level are disregarded
    // - Proto parent of this.fontOptions should be from the global options (i.e. as passed to network)
    //   TODO: Is this different from nodeOptions? Verify.
    // - newOptions.font (own props) should have been folded into this.fontOptions already
    //   TODO: verify this
    //
    //
    ...
    var getFontOptions = (multiName) => {
      ...
      for (var n in optionNames) {
        result[optionNames[n]] = getFontOption(multiName, optionNames[n]);
      }

      return result;
    }

    addFontToPile(newOptions);
    ...
    var assertModOptions = (a, b, mod) => {
      for (var option in b) {
        if (a[option] !== b[option]) {
          var msg = "Values of mod '" + mod + "' don't match for option: '" + option + "'. "
                  + "Original: '" + b[option] + "', new: '" + a[option] + "'";
    ...
    for (const mod of mods) {
      ...
      if (Label.parseFontString(modOptions, pathP(nodeOptions, mod))) {
        ...
        // don't want to sync to the base font face.
         modOptions.face =
          getP(nodeOptions     ,  mod, 'face')          || 
          getP(newOptions,  mod, 'face')          ||
           (mod === 'mono'? modDefaults.face:null ) ||
          getP(newOptions, null, 'face')          ||
          this.fontOptions.face
        ;

        // 'color' follows the standard flow
         modOptions.color =
          getP(nodeOptions     ,  mod, 'color') ||
          getP(newOptions,  mod, 'color') ||
          getP(newOptions, null, 'color') ||
          this.fontOptions.color
        ;

        // 'mode' follows the standard flow
        modOptions.mod =
          getP(nodeOptions     ,  mod, 'mod') ||
          getP(newOptions,  mod, 'mod') ||
          getP(newOptions, null, 'mod') ||
          modDefaults.mod
        ;
        ...
        modOptions.size =
          getP(nodeOptions     ,  mod, 'size')        ||
          getP(newOptions,  mod, 'size')        ||
          (ratio? modDefaults.size * ratio: null) || //   Scale the mod size using the same ratio
          getP(newOptions, null, 'size')        ||
          this.fontOptions.size
        ;


        modOptions.vadjust =
          getP(nodeOptions     , mod, 'vadjust')                     ||
          getP(newOptions, mod, 'vadjust')                     ||
          (ratio? modDefaults.vadjust * Math.round(ratio): null) || // Scale it using the same ratio
          this.fontOptions.vadjust
        ;

        assertModOptions(tmpModOptions, modOptions, mod);
        console.log("For " + mod + ":");
        console.log(tmpModOptions);

        // Copy our found values over (check if working)
