...
//      Label.parseOptions(this.fontOptions, options, allowDeletion);
  ...
  initFontOptions(options) {
    ...
    for (var n in options.font) {
      let prop = options.font[n];  // Use prototype values here as well!!!
  ...
  static parseOptions(parentOptions, newOptions, allowDeletion = false) {
    ...
    if (Label.parseFontString(parentOptions, newFont)) {
      ...
      //
      // TODO: replace with a decent recursive merge; fillIfDefined() is deficient.
...
*/
   ...
   *
   * **NOTE:** prototype handling of options is currently only present for
   *           backward compatibility for Edges. In due time, the need for
   ...
   *  - prototypes are only searched if this has been explicitly enabled for
   *    a given option object.
   *
   * ### Order:
   *
   *   - search in option group 'bold' in local properties
   *   - search in option group 'bold' in all properties, including prototypes
   *   - search in main font optiont group in local properties
   *   - search in main font optiont group in all properties, including prototypes
   *
   * ---------------------------------------------------------------------
   * ## Assumptions
   *
   ...
   */
  propagateFonts(pile) {
    if (!this.elementOptions.font.multi) return;
    ...
    var addFontToPile = (options, isFontOption = false, handleProto = false) => {
      ...
      fontPile.push([item, handleProto]);
    };
    ...
    */
    var getFontOption = (multiName, option) => {
      ...
      for (let n = 0; n < fontPile.length; ++n) {
        let [fontOptions, handleProto] = fontPile[n];

        if (fontOptions.hasOwnProperty(multiName)) {
          multiFont = fontOptions[multiName];
          if (multiFont !== undefined && multiFont.hasOwnProperty(option)) {
            return multiFont[option];
          }
        }
      }

      // Search multi font in prototypes also
      for (let n = 0; n < fontPile.length; ++n) {
        let [fontOptions, handleProto] = fontPile[n];
      ...
      // search font options in prototypes also
      for (let n = 0; n < fontPile.length; ++n) {
        let [fontOptions, handleProto] = fontPile[n];
    ...
    */
    ...
    //
    addFontToPile(pile[0]);                      // Newly set options on top
    //addFontToPile(this.elementOptions, true, true); // Options current instance
    //addFontToPile(this.fontOptions, true, true); // Options current instance
    for (let i = 1; i < pile.length; ++i) {      // Add the rest in order
    ...
    for (const mod of mods) {
      ...
      if (Label.parseFontString(modOptions, pathP(this.elementOptions, mod))) {
        let modDefaults = this.defaultOptions.font[mod];
