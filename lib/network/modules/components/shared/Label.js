...
class LabelAccumulator {
   ...
   * @private
   */
  add(l, text, width, height, values, mod = 'normal') {
...
class Label {
  ...
  _processLabelText(ctx, selected, hover, text) {
    let lines = new LabelAccumulator();
     ...
     */
     let append = function(text, mod = 'normal') { 
       addText(lines.current, text, mod);
     }
     ...
     */
    let newLine = function(text, mod = 'normal') { 
      addText(lines.current, text, mod);
      lines.current++;
    }
    ...
    /**
     * Determine the longest string which would still fit in the 
     * current max width.
     * 
    ...
    let splitStringIntoLines = function(str, mod = 'normal') {
      let words = str.split(" ");
      ...
      while (words.length > 0) {
        ...
        // In this case, just force using it; it will
        // be just that one word on a line
        if (w === 0) w = 1;
