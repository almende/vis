...
class LabelAccumulator {
   ...
   *
   * This method takes formatting values and measure var's out of the loops.
   *
   * @private
   *
   */
   addText(k, text, mod = 'normal') { 
     if (text === undefined) return;

     // Note that an empty block (width 0) will be added!
     let result = this.measureText(text, mod);
     this.add(k, text, result.width, result.values.height, result.values, mod);
   }
   ...
   append(text, mod = 'normal') { 
     this.addText(this.current, text, mod);
   }
...
class Label {
  ...
  _processLabelText(ctx, selected, hover, text) {
     ...
     * @return {Object} { width, values} width in pixels and return value of measuring
     */
