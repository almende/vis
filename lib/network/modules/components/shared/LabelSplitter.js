...
class MarkupAccumulator {
  ...
  /**
   * @param {string} text  full text that is being parsed
   * @param {string} tagName label for block type we are currently processing
   * @param {string|RegExp} tag string to match in text
   * @returns {boolean} true if the tag was processed, false otherwise
   */
  parseStartTag(text, tagName, tag) {
    let [regExp, length] = this.prepareRegExp(tag);

    // Note: if 'mono' passed as tagName, there is a double check here. This is OK
    if (!this.mono && !this[tagName] && regExp.test(text.substr(this.position, length))) {
      this.setTag(tagName);
   ...
   * @param {string} tagName label for block type we are currently processing
   * @param {string|RegExp} tag string to match in text
   * @param {RegExp} [nextRegExp] regular expression to match for characters *following* the current tag 
   * @returns {boolean} true if the tag was processed, false otherwise
   */
  parseEndTag(text, tagName, tag, nextRegExp) {
    let ch = text.charAt(this.position);
    let [regExp, length] = this.prepareRegExp(tag);
    ...
    if (checkTag && regExp.test(text.substr(this.position, length))) {
      if (nextRegExp !== undefined) {
        if ((this.position === text.length-1) || nextRegExp.test(text.charAt(this.position + 1))) {
   ...
   * @returns {boolean} true if the tag was processed, false otherwise
   */
  replace(text, tag, value) {
    let [regExp, length] = this.prepareRegExp(tag);

    if (regExp.test(text.substr(this.position, length))) {
      this.add(value);
      this.position += length - 1;
      return true;
    }
  ...
  prepareRegExp(tag) {
    ...
    if (tag instanceof RegExp) {
      regExp = tag;
      length = 1;   // ASSUMPTION: reg ex only tests one character
    } else {
...
class LabelSplitter {
  ...
  splitHtmlBlocks(text) {
    ...
    let parseEntities = (text, ch) => {
      if (/&/.test(ch)) {
        let parsed = s.replace(text, '&lt;', '<')
          || s.replace(text, '&amp;', '&');
    ...
    while (s.position < text.length) {
      ...
      let parsed = s.parseWS(ch)
        || (/</.test(ch) && ( 
             s.parseStartTag(text, 'bold', '<b>')
          || s.parseStartTag(text, 'ital', '<i>')
          || s.parseStartTag(text, 'mono', '<code>')
          || s.parseEndTag(text, 'bold', '</b>')
          || s.parseEndTag(text, 'ital', '</i>')
          || s.parseEndTag(text, 'mono', '</code>')))
        || parseEntities(text, ch);
  ...
  splitMarkdownBlocks(text) {
    ...
    while (s.position < text.length) {
      ...
      let parsed = s.parseWS(ch)
        || parseOverride(ch)
        || ((beginable || s.spacing) && (
             s.parseStartTag(text, 'bold', /\*/)
          || s.parseStartTag(text, 'ital', '_')
          || s.parseStartTag(text, 'mono', '`')))
        || s.parseEndTag(text, 'bold', /\*/, /[.,_` \t\n]/)
        || s.parseEndTag(text, 'ital',  '_', /[.,*` \t\n]/)
        || s.parseEndTag(text, 'mono',  '`', /[.,*_ \t\n]/);
