...
class CircleImageBase extends NodeBase {
  ...
  setOptions(options, imageObj, imageObjAlt) {
    ...
    //
    // The issue here is that this setImages() is called twice on initialization:
    //
    // - First here, via the constructor call in parent NodeBase
    // - Second in the constructor in derived Image or CircleImage.
    //
    // As an extra complication, this setOptions() is also called
    // on an update to the node, where the two images are passed
    // for Image and CircleImage (only).
    //
    // In the first case, imageObj and ImageObjAlt are always undefined;
    // we use this to differentiate the calls.
  ...
  setImages(imageObj, imageObjAlt) {
    if (imageObj === undefined) {
      throw "imageObj must be defined";
    }
