...
class DirectionInterface {
   ...
   *
   * **Note:** chrome has a tendency to change the order of the 
   * array items anyway if the custom sort function returns 0.
   * This sort forces the issue by falling back to the array order
   * if the compare values can't be determined.
   *
  ...
  /**
   * Map node id's in array to their position within the array
   *
   * Sort changes the array inline, we need to keep track of id order separately.
   *
   * @param {Array.<Node>}  nodeArray array of nodes
   * @returns {object.<NodeId, number>}  hash with node id's to array index
   */
  idPerLevel(nodeArray) {
    let idOrder = {};
    for (var n = 0; n < nodeArray.length; ++n) {
      let item = nodeArray[n];
      idOrder[item.id] = n;
    }
...
class VerticalStrategy extends DirectionInterface {
  ...
  /** @inheritdoc */
  sort(nodeArray) {
    let idOrder = this.idPerLevel(nodeArray);
