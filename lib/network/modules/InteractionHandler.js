...
var ComponentUtil = require('./components/shared/ComponentUtil').default;
...
class InteractionHandler {
 ...
 _checkHidePopup(pointer) {
  ...
  /**
   * Determine the topmost label, if any, at the given pointer position.
   *
   * The return value consists of separate hashes for nodes and edges, in which
   * the keys are the id's of the node or the hash, and the value is the id for
   * the label clicked on.
   *
   * @param {point} pointer  mouse position in screen coordinates
   * @returns {{nodes: {number: [number]}, edges: {number: [number]}}}
   *
   * NOTES
   *
   * - Currently, there is only one label associated with a node or an edge,
   *   but this is expected to change somewhere in the future.
   * - To take it even further into the future, it may be required to return multiple
   *   values.
   *
   * The given construct gives the flexibility to allow for multiple labels and multiple
   * return values per click. 
   */ 
  getClickedLabels(pointer) {
    ...
    for (let i = 0; i < nodeIndices.length; i++) {
      ...
      if(ComponentUtil.overlaps(node.getLabelSize(), point)) {
        overlappingNodes.push(nodeIndices[i]);
      }
    }
    ...
    // Last node is topmost
    if (overlappingNodes.length !== 0) {
      // Refer to the id of the label
      // The idea here is to allow for multiple labels later on for nodes and edges
      let lastNode = overlappingNodes[overlappingNodes.length -1];
      ret.nodes[lastNode.id] = 0;  // value is label id
    }
