...
class InteractionHandler {
  ...
  isInSelectionBoxState() {
    if (Object.keys(this.body.selectionBox).length > 0) {
      return true;
    }
    else {
      return false;
  ...
  onTouch(event) {    
    if (new Date().valueOf() - this.touchTime > 50) {
      if (event.srcEvent.ctrlKey) {
        ...
        this.body.selectionBox["x"] = p.x;
        this.body.selectionBox["y"] = p.y;

        this.body.selectionBox["width"] = 0;
        this.body.selectionBox["height"] = 0;

        console.log("SELECTION BOX STATE: " + this.isInSelectionBoxState());
      }
  ...
  onRelease(event) {
    if (new Date().valueOf() - this.touchTime > 10) {
      if (this.isInSelectionBoxState()) {
        console.log("Clearing selection box state");
        this.body.selectionBox = {};
      }
      let pointer = this.getPointer(event.center);
  ...
  onDrag(event) {
    ...
    if (this.isInSelectionBoxState()) {
      console.log("MUTATING SELECTION BOX");      
      let p = this.canvas.DOMtoCanvas({
        x: event.srcEvent.offsetX,
        y: event.srcEvent.offsetY
      });
      
      let xDir = this.body.selectionBox.x <= p.x ? 1 : -1;
      let yDir = this.body.selectionBox.y <= p.y ? 1 : -1;

      this.body.selectionBox["width"] = Math.abs(p.x - this.body.selectionBox.x) * xDir;
      this.body.selectionBox["height"] = Math.abs(p.y - this.body.selectionBox.y) * yDir;
      //console.log("ORIGIN: " + this.body.selectionBox.x + ", " + this.body.selectionBox.y + " | nx: " + (p.x * xDir) + ", ny: " + (p.y * yDir));
