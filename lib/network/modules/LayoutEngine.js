 ...
 *
 * Conversely: in a network that is not a tree, there exist nodes with multiple parent.
 * Also, by definition: a network consisting of unconnected sub-networks, of which
 * at least one is not a tree, is not a forest.
 *
 * The distinction between tree and non-tree is important in the code, notably within
 * to the block-shifting algorithm. This assumes trees and fails for non-trees, often
 * in a spectacular manner (search for 'exploding network' in the issues).
 *
 * Note: A hierarchical layout is a different thing from a hierarchical network.
...
class LayoutEngine {
  ...
  setupHierarchicalLayout() {
    if (this.options.hierarchical.enabled === true && this.body.nodeIndices.length > 0) {
      ...
      // After initialization: Highest tree id in current network.
      // So, the number of separate trees is (treeIndex + 1).
      this.treeIndex = -1;
