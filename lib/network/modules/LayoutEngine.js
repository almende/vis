...
class LayoutEngine {
  ...
  setDirectionStrategy() {
    ...
    if(isVertical) {
      this.direction = {
        ...
        // Dynamic curves do not work for there.
        type: 'vertical',
        ...
        // Fix the 'other' coordinate for the direction 
        fix: (node, level) => {
        ...
        shift: (nodeId, diff) => {
          this.body.nodes[nodeId].x += diff;
        },
      };
    } else {
      // See the vertical instance for comments.
      this.direction = {
        type: 'horizontal',
  ...
  _sortNodeArray(nodeArray) {
    if (nodeArray.length > 1) {
      nodeArray.sort(this.direction.sort);
    }
  }
