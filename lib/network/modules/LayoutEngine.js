...
class LayoutEngine {
   ...
   *
   * Node levels are strictly incremented in the to-direction.
   * Going back a level is thus blocked.
   *
   * @param hubsize
   * @private
   */
  _determineLevelsDirected() {
    ...
    let levelByDirection = (nodeA, nodeB, edge) => {
      // This callback gets called twice, for A-B and B-A
      // Only deal with A -> B here
      if (edge.toId === nodeA.id) return;

      let levelA = this.hierarchical.levels[nodeA.id];
      // set initial level
      if (levelA === undefined) { levelA = this.hierarchical.levels[nodeA.id] = minLevel;}

      if (this.hierarchical.levels[nodeB.id] === undefined) {
